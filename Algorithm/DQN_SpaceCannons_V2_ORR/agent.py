...
def main(test=False, chkpt=None, device = 'cuda', boltzmann_expo = True):
    ...
    try:
        while Game_num!=1000:
            ...
            rb1.insert(SARSD(last_observation, action1, reward[0], observation, done, bufferID_A1))
            # rb1.print_buffer()
            # Update replaybuffer Agent 2
            rb2.insert(SARSD(last_observation, action2, reward[1], observation, done, bufferID_A2))

            if info['coop_kill_miniboss'] == 1:
                rb1.redistribute_reward(reward[0])
                rb2.redistribute_reward(reward[1])
            # update observations
