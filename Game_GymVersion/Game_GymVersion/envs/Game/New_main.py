...
while running:
    ...
    if bullet_count_red <= 0:
        ...
        image_cooldown = pygame.transform.scale(image_cooldown, (80,80))
    ...
    if bullet_count_blue <= 0:
        ...
        image_cooldown_b = font_b.render(str(diff_b), True, (16,52,166))
        image_cooldown_b = pygame.transform.scale(image_cooldown_b, (80,80))
...
# Collision detection
    ...
    if bullet_count_red <= 0:
        ...
        diff = ((now - last)/1000)
        font = pygame.font.Font(None, 100)
        image_cooldown = font.render(str(diff), True, (255,0,0))
        image_cooldown = pygame.transform.scale(image_cooldown, (40,40))
        rect = image_cooldown.get_rect()
        rect.x = 642
        rect.y = 570
        display.blit(image_cooldown, (rect.x,rect.y))
    ...
    if bullet_count_blue <= 0:
        ...
        font_b = pygame.font.Font(None, 100)
        image_cooldown_b = font_b.render(str(diff_b), True, (16,52,166))
        image_cooldown_b = pygame.transform.scale(image_cooldown_b, (40,40))
        rect_b = image_cooldown_b.get_rect()
        rect_b.x = 255
        rect_b.y = 570
        display.blit(image_cooldown_b, (rect_b.x,rect_b.y))
